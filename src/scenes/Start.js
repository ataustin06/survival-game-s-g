...
buildSurplusOnlyPartialAllocation ()
{
    ...
    const donors = startingAllocation
        .map((amount, index) => ({
            index,
    ...
    const totalAvailableSurplus = donors.reduce(
        (sum, donor) => sum + donor.surplus,
        0
    );

    const recipients = startingAllocation
        .map((amount, index) => ({
            index,
    ...
    const contributionShares = donors.map(donor => {
        ...
        const baseContribution = Math.floor(
            exactContribution
        );
        ...
        return {
            ...
            remainder:
                exactContribution - baseContribution
        };
    });
    ...
    while (
        assignedContributions < totalTransferRequired
    )
    {
        const donor = contributionShares[
            donorCursor % contributionShares.length
        ];
